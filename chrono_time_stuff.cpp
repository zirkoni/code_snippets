...
std::chrono::system_clock::duration calculateDelay(uint16_t delayDays, uint16_t minutesFromMidnight, uint16_t secondsFromMidnight)
{
    ...
    // Simpler print
    const std::time_t tPrint = std::chrono::system_clock::to_time_t(tnow);
    std::cout << "Date now:     " << std::ctime(&tPrint);
    ...
    const std::time_t tPrint2 = std::chrono::system_clock::to_time_t(triggerDateAndHour);
    std::cout << "Date now:     " << std::ctime(&tPrint2);
